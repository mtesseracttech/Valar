...
namespace mt::gfx {
void WindowCallbackManager::register_window(
    std::weak_ptr<gfx::ContextWindow> window)
{
    if (!window.expired()) {
        auto win = window.lock();
        Logger::log("Registering Callbacks for Window '" + win->get_title() + "'",
            Info);
        glfwSetWindowUserPointer(win->get_window(), this);
        // Setting the general GLFW error callback
        glfwSetErrorCallback(window_error_callback);
        // Setting the GLFW FrameBuffer Resize callback, that is called whenever the
        // window changes sizes
        glfwSetFramebufferSizeCallback(win->get_window(),
...
void WindowCallbackManager::deregister_window(
    std::weak_ptr<gfx::ContextWindow> window)
{
    if (!window.expired()) {
        auto win = window.lock();
        Logger::log("Deregistering Callbacks for Window '" + win->get_title() + "'",
            Info);
        window_callbacks.erase(win->get_window());
    }
}

void WindowCallbackManager::framebuffer_resize_callback(GLFWwindow* window,
...
void WindowCallbackManager::window_error_callback(int error_code,
...
void WindowCallbackManager::keyboard_callback(GLFWwindow* window, int key,
...
void WindowCallbackManager::mouse_button_callback(GLFWwindow* window,
...
void WindowCallbackManager::mouse_pos_callback(GLFWwindow* window, double x,
    double y)
{
    auto manager = get_callback_manager(window);
    if (manager) {
...
std::optional<WindowCallbackManager>
WindowCallbackManager::get_callback_manager(GLFWwindow* window)
{
    if (window) {
        auto manager = glfwGetWindowUserPointer(window);
        if (manager)
            return *reinterpret_cast<WindowCallbackManager*>(manager);
    }
    return {};
}
} // namespace mt::gfx