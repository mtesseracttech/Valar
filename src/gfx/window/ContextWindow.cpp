...
ContextWindow::ContextWindow(uint32_t width, uint32_t height,
...
void ContextWindow::framebuffer_resized(uint32_t new_width,
    uint32_t new_height)
{
    width = new_width;
    height = new_height;
...
GLFWwindow* ContextWindow::get_window() const { return window; }

uint32_t ContextWindow::get_width() const { return width; }

uint32_t ContextWindow::get_height() const { return height; }
...
std::string ContextWindow::get_title() const { return title; }

void ContextWindow::set_renderer(
    const std::weak_ptr<mtvk::VulkanRenderer>& renderer)
{
