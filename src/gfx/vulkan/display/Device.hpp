...
public:
    explicit Device(const std::shared_ptr<Surface>& surface)
        : surface(surface)
    {
    }
    ...
    {
        auto best_physical_device = select_best_physical_device(
            instance.get_instance().enumeratePhysicalDevices(),
            surface->get_surface());
        auto logical_device = create_logical_device(best_physical_device, instance,
            surface->get_surface());
    ...
    // Call this to destroy the vulkan device, at the very end of all vulkan
    // activity
    void destroy()
    {
    ...
    void wait_till_idle() { device.waitIdle(); }

    // Returns the command queue indices
    QueueFamilyIndices get_queue_indices() const { return indices; }
    ...
    vk::Queue get_graphics_queue() const { return graphics_queue; }

    vk::Queue get_present_queue() const { return present_queue; }

private:
    static vk::PhysicalDevice select_best_physical_device(
        ...
        const vk::SurfaceKHR& surface)
    {
        ...
        for (const auto& device : physical_devices) {
            if (is_physical_device_suitable(device, surface)) {
                // ranked_devices.push(std::make_pair(rate_physical_device(device),
                // device));
        ...
        if (ranked_devices.empty()) {
            ...
            throw std::runtime_error(
                "Failed to find a device that satisfies all hard requirements!");
        }
        ...
        while (!ranked_devices.empty()) {
            ...
            std::stringstream ss;
            ss << i++ << ". " << ranked_device.second.getProperties().deviceName
               << " (" << ranked_device.first << ")";
            Logger::log(ss, Info);
            ranked_devices.pop();
        }

        Logger::log("Selected the GPU: " + std::string(best_device.getProperties().deviceName),
            Info);
    ...
    static vk::Device create_logical_device(
        const vk::PhysicalDevice& physical_device, const Instance& instance,
        const vk::SurfaceKHR& surface)
    {
    ...
    static bool check_device_extension_support(const vk::PhysicalDevice& device)
    {
        ...
        std::set<std::string> required_extensions(ext::device_extensions.begin(),
            ext::device_extensions.end());
