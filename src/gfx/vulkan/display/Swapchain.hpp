...
public:
    ...
    vk::Extent2D get_extent() const { return extent; }
    vk::Format get_format() const { return image_format; }

    vk::SwapchainKHR get_swapchain() const { return swapchain; }

    std::vector<vk::Framebuffer> get_framebuffers() const { return framebuffers; }

    std::vector<vk::Image> get_images() const { return images; }
...
private:
        ...
        const std::vector<vk::SurfaceFormatKHR>& available_formats)
    {
        ...
        return available_formats[0]; // If nothing else works, just pick the first
            // one
    }
