...
public:
    ...
    public:
        ShaderMetaModule(const vk::ShaderStageFlagBits& stage,
            const vk::ShaderModule& module, const uint32_t* data,
            uint32_t size)
            : stage(stage)
            , module(module)
...
public:
    explicit Shader(const std::shared_ptr<Device>& device,
    ...
    create_shader_stage_create_infos() const;
    ...
    struct ShaderModuleWithMeta {
        ...
        ShaderModuleWithMeta(vk::ShaderStageFlagBits stage, vk::ShaderModule module,
            const uint32_t* spirv_data, uint32_t spirv_size)
            : stage(stage)
            , module(module)
            , reflection_data(spirv_data, spirv_size)
        {
        }
    };

private:
    std::vector<ShaderSourceData> find_sources(
