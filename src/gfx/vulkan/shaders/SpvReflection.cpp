...
SpvReflection::SpvReflection(const uint32_t* data, uint32_t size)
{
    ...
    resources = std::make_shared<spirv_cross::ShaderResources>(
        compiler->get_shader_resources());
}

std::shared_ptr<spirv_cross::ShaderResources>
SpvReflection::get_shader_reflection_resources() const
{
    return resources;
}

spirv_cross::SPIRType SpvReflection::get_type_info(
    const spirv_cross::TypeID id) const
{
    return compiler->get_type(id);
}

size_t SpvReflection::get_struct_member_size(const spirv_cross::SPIRType id,
...
std::size_t SpvReflection::get_base_type_size(
