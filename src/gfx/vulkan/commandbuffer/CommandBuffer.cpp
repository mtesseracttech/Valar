...
void CommandBuffer::allocate_command_buffers(const Swapchain& swapchain)
{
    ...
    device->get_device().allocateCommandBuffers(&command_buffer_allocate_info,
        command_buffers.data());
    Logger::log("Allocated " + std::to_string(command_buffers.size()) + " Command Buffers",
        Info);
}
    ...
    const RenderPass& render_pass)
{
    ...
    for (std::size_t i = 0; i < command_buffers.size(); ++i) {
        ...
        command_buffers[i].bindPipeline(pipeline.get_bind_point(),
...
{
    device->get_device().waitForFences(1, &in_flight_fences[current_frame], true,
        UINT64_MAX);

    auto next_image = device->get_device().acquireNextImageKHR(
    ...
    if (images_in_flight[next_image.value]) {
        device->get_device().waitForFences(1, &images_in_flight[next_image.value],
            true, UINT64_MAX);
    }
